...
export const EstudianteEntregas: React.FC = () => {
      ...
      if (res.ok) {
        ...
        const listaMapeada: Entrega[] = listaRaw.map((a: any) => {
          ...
          return {
            ...
            description: a.description || "Sub reporte de hallazgos y evidencias.",
  ...
  useEffect(() => {
    if (!selectedEntrega?.id) return;

    const token = localStorage.getItem("token");
    fetch(`${API_ACTIVIDADES_URL}/${selectedEntrega.id}/evidencias`, {
      headers: { Authorization: `Bearer ${token}` }
    })
      .then(res => res.json())
      .then(data => {
        ...
        setSelectedEntrega(prev => prev ? { ...prev, evidence: listaEv } : null);
        setEntregas(prevList => prevList.map(item => 
          item.id === selectedEntrega.id ? { ...item, evidence: listaEv } : item
        ));
      })
      .catch(err => console.error("Error al cargar evidencias de la entrega:", err));
  }, [selectedEntrega?.id]);
  ...
  const abrirEvidencia = (url: string) => {
  ...
  return (
    <div className="p-6 space-y-5 w-full font-sans antialiased text-gray-900 box-border">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3.5 w-full">
        <StatCard label="Total" value={counts.total} color="bg-indigo-50 text-indigo-600" icon={<Package size={18} />} />
        <StatCard label="Pendientes" value={counts.pendientes} color="bg-gray-50 text-gray-400" icon={<Clock size={18} />} />
        <StatCard label="En Revisión" value={counts.enRevision} color="bg-amber-50 text-amber-600" icon={<Eye size={18} />} />
        <StatCard label="Aprobados" value={counts.aprobados} color="bg-emerald-50 text-emerald-600" icon={<CheckCircle2 size={18} />} />
        <StatCard label="Completadas" value={counts.completadas} color="bg-green-50 text-green-600" icon={<CheckCircle2 size={18} />} />
      </div>
...
export default EstudianteEntregas;