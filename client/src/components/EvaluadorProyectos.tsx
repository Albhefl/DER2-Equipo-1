...
import { SidebarEvaluador } from './SidebarEvaluador'; // 1. Traemos el menú global
import { 
  Search,
  Info
} from 'lucide-react';
...
export const EvaluadorProyectos: React.FC = () => {
  ...
  return (
    <div className="flex bg-[#F8FAFC] min-h-screen font-sans antialiased text-[#111827]">
      ...
      {/* 2. Colocamos el Sidebar reutilizable limpio */}
      <SidebarEvaluador />
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-[72px] bg-white border-b border-gray-100 flex items-center justify-between px-8 shrink-0">
          <div>
            <h2 className="text-lg font-bold text-gray-900 leading-none mb-1">Proyectos asignados</h2>
                  ...
                  {proyectosFiltrados.length > 0 ? (
                    proyectosFiltrados.map((p) => (
                      <tr key={p.id} className="hover:bg-gray-50/30 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          {/* 3. El link interactivo ahora sí está en la celda correspondiente */}
