...
import { Link } from 'react-router-dom'; // 1. Cambiamos las herramientas de importación por Link
import { SidebarEvaluador } from './SidebarEvaluador'; // 2. Importamos tu nuevo menú global
import { 
  Folder, 
  Bell, 
  CheckSquare, 
...
export const EvaluadorDashboard: React.FC = () => {
  ...
  return (
    <div className="flex bg-[#F8FAFC] min-h-screen font-sans antialiased text-[#111827]">
      ...
      {/* 3. REEMPLAZO DEL ASIDE VIEJO POR EL COMPONENTE INTELIGENTE */}
      <SidebarEvaluador />
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-[72px] bg-white border-b border-gray-100 flex items-center justify-between px-8 shrink-0">
          <div>
            <h2 className="text-lg font-bold text-gray-900 leading-none mb-1">Dashboard del Evaluador</h2>
        ...
        <main className="p-8 flex flex-col lg:flex-row gap-6 overflow-y-auto">
          ...
          <div className="flex-1 space-y-6 min-w-0">
            ...
            {/* TABLA DE PROYECTOS */}
            <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm shadow-gray-100/40">
              <div className="p-5 border-b border-gray-100 flex justify-between items-center">
                <h3 className="font-bold text-gray-900 text-[15px]">Proyectos asignados</h3>
                {/* 4. Cambiamos el <button> estático por el <Link> interactivo */}
                  ...
                  <tbody className="divide-y divide-gray-100 text-[13px] text-gray-700">
                    {proyectos.map((p) => (
                      <tr key={p.id} className="hover:bg-gray-50/30 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          {/* 5. Cambiamos el <a> vacío por el <Link> a la pantalla de detalle */}
