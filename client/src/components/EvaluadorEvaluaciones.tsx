...
export const EvaluadorEvaluaciones: React.FC = () => {
  ...
  return (
    <div className="flex bg-[#F8FAFC] min-h-screen font-sans antialiased text-[#111827]">
      ...
      {/* 3. Reemplazo del aside por el Sidebar unificado */}
      <SidebarEvaluador />
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="p-8 space-y-6 overflow-y-auto">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm shadow-gray-100/40 min-h-[400px]">
            
            {/* SECCIÓN DE FILTROS CON DESPLEGABLES */}
            <div className="p-5 border-b border-gray-100 flex flex-col xl:flex-row gap-4 justify-between items-start xl:items-center relative z-20">
              ...
              <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
                ...
                <div className="relative">
                  <button 
                    ...
                    className="flex items-center justify-between gap-4 bg-white border border-gray-100 px-3 py-1.5 rounded-xl text-xs text-gray-600 font-medium hover:bg-gray-50 min-w-[150px]"
                  >
                    <span>{filtroProyecto}</span> <ChevronDown size={14} className="text-gray-400" />
                  </button>
                  {openProyecto && (
                ...
                <div className="relative">
                  <button 
                    ...
                    className="flex items-center justify-between gap-4 bg-white border border-gray-100 px-3 py-1.5 rounded-xl text-xs text-gray-600 font-medium hover:bg-gray-50 min-w-[140px]"
                  >
                    <span>{filtroEstado}</span> <ChevronDown size={14} className="text-gray-400" />
                  </button>
                  {openEstado && (
                ...
                <div className="relative">
                  <button 
                    ...
                    className="flex items-center justify-between gap-4 bg-white border border-gray-100 px-3 py-1.5 rounded-xl text-xs text-gray-600 font-medium hover:bg-gray-50 min-w-[130px]"
                  >
                    <span>{filtroEtapa}</span> <ChevronDown size={14} className="text-gray-400" />
                  </button>
                  {openEtapa && (
                    <div className="absolute left-0 mt-1 w-44 bg-white border border-gray-100 rounded-xl shadow-lg py-1 z-30">
                  ...
                  {evaluacionesFiltradas.length > 0 ? (
                    evaluacionesFiltradas.map((ev) => (
                      <tr key={ev.id} className="hover:bg-gray-50/30 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          {/* 4. Cambiamos el <a> estático por el <Link> dinámico al formulario */}
                          {ev.isLink ? (
