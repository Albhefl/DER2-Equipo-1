...
import { Link } from 'react-router-dom'; // 1. Importamos Link de React Router
import { SidebarEvaluador } from './SidebarEvaluador'; // 2. Importamos el menú lateral global
import { 
  ArrowLeft,
  ShieldCheck,
  CheckCircle2,
...
export const EvaluadorDetalleProyecto: React.FC = () => {
  ...
  return (
    <div className="flex bg-[#F8FAFC] min-h-screen font-sans antialiased text-[#111827]">
      ...
      {/* 3. Reemplazo por el Sidebar inteligente reutilizable */}
      <SidebarEvaluador />
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* ÁREA DE CONTENIDO */}
        <main className="p-8 flex flex-col lg:flex-row gap-6 overflow-y-auto">
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm shadow-gray-100/40 space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-gray-400">Nombre: Elías <span className="mx-1">|</span> Carlos Flores / Evaluador</p>
                </div>
              </div>

              <div className="flex flex-wrap items-center gap-y-2 gap-x-6 pt-2 text-xs">
                <p className="text-gray-500 font-medium">Fecha de entrega: <span className="text-gray-900 font-bold ml-1">Viernes 6 Jun</span></p>
                <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-blue-50 text-blue-600 border border-blue-100">En proceso</span>
                <div className="flex items-center gap-2 flex-1 min-w-[150px]">
                  <span className="text-gray-400 font-medium shrink-0">Progreso: 75%</span>
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm shadow-gray-100/40 space-y-4">
              ...
              <div className="space-y-3.5">
                {progresoDetallado.map((item) => (
                  <div key={item.tarea} className="flex items-center gap-4 text-xs">
                    <p className="w-36 font-semibold text-gray-600 shrink-0">{item.tarea}</p>
                    <div className="flex-1 bg-gray-100 h-2 rounded-full overflow-hidden">
                      <div className={`${item.color} h-2 rounded-full`} style={{ width: `${item.porcentaje}%` }}></div>
                    </div>
                    <span className="w-10 text-right text-gray-400 font-bold">{item.porcentaje}%</span>
                  </div>
                ))}
              </div>
            </div>
